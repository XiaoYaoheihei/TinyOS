...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos) {
  ...
  // I 结点相对于 inode_table_lba 的字节偏移量
  uint32_t off_size = inode_no * inode_size;
  //I 结点相对于 inode_table_lba 的扇区偏移量
...
struct inode* inode_open(struct partition* part, uint32_t inode_no) {
  ...
  //从硬盘上读入此 inode 并加入到此链表
  struct inode_position inode_pos;
  //inode 位置信息会存入 inode_pos,包括 inode 所在扇区地址和扇区内的字节偏移量
  inode_locate(part, inode_no, &inode_pos);
  //为使通过 sys_malloc 创建的新 inode 被所有任务共享
  //需要将 inode 置于内核空间，不应该放在用户内存池中应该内核内存池中
  //故需要临时将 cur_pbc->pgdir 置为 NULL
  ...
  if (inode_pos.two_sec) {
    ...
    //i 结点表是被 partition_format 函数连续写入扇区,可以连续读出来
    ide_read(part->my_disk, inode_pos.sec_lba, inode_buf, 2);
  } else {
    //查找的inode没有跨扇区，一个扇区大小的缓冲区足够
...
//在 inode_table 中把该 inode 对应的空间清 0
...
//回收 inode 的数据块和 inode 本身在 inode 位图中的 bit
void inode_release(struct partition* part, uint32_t inode_no) {
  ...
  //步骤1：先将前 12 个直接块存入 all_blocks
  while (block_idx < 12) {
    all_blocks[block_idx] = inode_to_del->i_sectors[block_idx];
    block_idx++;
  }
  //步骤2：如果一级间接块表存在，将其 128 个间接块读到 all_blocks[12~]，
  //并释放一级间接块表所占的扇区
  ...
  //步骤3：inode 所有的块地址已经收集到 all_blocks 中，下面逐个回收
  block_idx = 0;
  ...
  //此函数会在 inode_table 中将此 inode 清 0,但实际上是不需要的
  //inode 分配是由 inode 位图控制的,硬盘上的数据不需要清 0，可以直接覆盖
