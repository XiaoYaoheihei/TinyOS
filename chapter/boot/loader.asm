...
;----------创建页目录以及页表并且初始化页内存位图
	...
	sgdt [gdt_ptr]		;将GDTR寄存器中存储的地址以及偏移量写入内存gdt_ptr中
	...
	lgdt [gdt_ptr]										;现在已经正式启动分页机制
																		;开启分页之后，重新加载gdt的地址
																		;将gdt_ptr处内存的值重新加载到GDTR寄存器中
	;在开启分页之后，GDT的基址会变成3GB之上的虚拟地址
	;而显存段基址也会变成3GB之上的虚拟地址
	;gs中存储的选择子，现在GDTR中拿到基址，和选择子中的偏移值相加，因为是虚拟值
	;所以需要经过一次页表映射，找到对应的视频段的基址
	;此时视频段的基址+160还是虚拟值，再经过一次页表
	;得到真正的物理地址0xb8000，将具体的值写入相应的物理地址处
