...
// 此结构在线程自己的内核栈中位置不固定
struct thread_stack{
  //位于 Intel386 硬件体系上的所有寄存器都具有全局性,在函数调用过程中，这5个寄存器对于
  //主调函数和被调函数都可见，这5个寄存器归主调函数所有，其余的寄存器归被调函数所有
  //也就是说被调函数必须为主调函数保护好这5个寄存器的内容，被调函数执行完之后这5个寄存器的内容
  //必须和执行被调函数之前一样，他必须在栈中存储这些值。
  //schedule函数进行调用切换的时候将这5个寄存器的值保存在线程栈中。
  uint32_t ebp;
  uint32_t ebx;
  uint32_t edi;
  uint32_t esi;
